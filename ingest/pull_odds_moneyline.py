import argparse
import json
import os
import time
from datetime import date, datetime, timedelta, timezone

import psycopg2
from odds.the_odds_api import TheOddsAPI


def try_map_game_id(cur, game_date, home_name, away_name):
    cur.execute(
        """
        SELECT game_id
        FROM games
        WHERE game_date = %s
          AND home_team_name = %s
          AND away_team_name = %s
        LIMIT 1;
        """,
        (game_date, home_name, away_name),
    )
    row = cur.fetchone()
    return row[0] if row else None


def utc_day_window(game_date):
    day_start = datetime.combine(game_date, datetime.min.time(), tzinfo=timezone.utc)
    day_end = day_start + timedelta(days=1)
    return (
        day_start.isoformat().replace("+00:00", "Z"),
        day_end.isoformat().replace("+00:00", "Z"),
    )


def process_date(client, game_date, args, conn):
    print(f"\nProcessing date: {game_date}")

    snapshot_dt = datetime.combine(game_date, datetime.min.time().replace(hour=12))
    snapshot_dt = snapshot_dt.replace(tzinfo=timezone.utc)
    snapshot_iso = snapshot_dt.isoformat().replace("+00:00", "Z")

    print(f"  Fetching historical odds snapshot at: {snapshot_iso}")

    try:
        data = client.get_moneylines_historical(snapshot_iso=snapshot_iso)
    except Exception as e:
        print(f"Error fetching odds for {game_date}: {e}")
        return 0

    print(f"Events returned from API: {len(data)}")
    if data:
        print(f"First event keys: {list(data[0].keys())}")
        print(f"First commence_time: {data[0].get('commence_time')}")
        print(f"First home/away: {data[0].get('home_team')} vs {data[0].get('away_team')}")

    sb_allow = set(
        s.strip().lower()
        for s in args.bookmakers.split(",")
        if s.strip()
    ) if args.bookmakers else set()

    rows = []
    for ev in data:
        commence = ev.get("commence_time")
        if not commence:
            continue

        try:
            if commence.endswith("Z"):
                commence_dt = datetime.fromisoformat(commence.replace("Z", "+00:00"))
            else:
                commence_dt = datetime.fromisoformat(commence)

            commence_date = commence_dt.astimezone(timezone.utc).date() if commence_dt.tzinfo else commence_dt.date()
            if commence_date != game_date:
                continue
        except (ValueError, AttributeError) as e:
            if not str(commence).startswith(str(game_date)):
                continue
            print(f"  Warning: Could not parse commence_time '{commence}': {e}")

        home = ev.get("home_team")
        away = ev.get("away_team")
        if not home or not away:
            continue

        for book in ev.get("bookmakers", []) or []:
            sb = (book.get("title") or book.get("key") or "").strip()
            if not sb:
                continue
            if sb_allow and sb.lower() not in sb_allow:
                continue

            markets = book.get("markets", []) or []
            h2h = next((m for m in markets if m.get("key") == "h2h"), None)
            if not h2h:
                continue

            outcomes = h2h.get("outcomes", []) or []
            home_price = None
            away_price = None

            for o in outcomes:
                name = o.get("name")
                price = o.get("price")
                if name == home:
                    home_price = price
                elif name == away:
                    away_price = price

            rows.append({
                "game_date": str(game_date),
                "provider": args.provider,
                "sportsbook": sb,
                "market": "h2h",
                "home_team": home,
                "away_team": away,
                "home_price": home_price,
                "away_price": away_price,
                "commence_time": commence,
                "raw": ev,
            })

    if not rows:
        print(f"No odds rows found for date: {game_date}")
        return 0

    inserted = 0
    skipped_unmapped = 0

    with conn.cursor() as cur:
        for r in rows:
            game_id = try_map_game_id(cur, r["game_date"], r["home_team"], r["away_team"])
            if game_id is None:
                skipped_unmapped += 1
                print(
                    f"  Warning: could not map game_id for "
                    f"{r['away_team']} @ {r['home_team']} on {r['game_date']}"
                )
                continue

            cur.execute(
                """
                INSERT INTO odds_ml_rebuild (
                    game_date, game_id, provider, sportsbook, market,
                    home_team, away_team, home_price, away_price, commence_time, raw
                )
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s::jsonb)
                ON CONFLICT (game_id, provider, sportsbook, market, commence_time)
                DO UPDATE SET
                    home_price = EXCLUDED.home_price,
                    away_price = EXCLUDED.away_price,
                    raw = EXCLUDED.raw,
                    home_team = EXCLUDED.home_team,
                    away_team = EXCLUDED.away_team,
                    game_date = EXCLUDED.game_date;
                """,
                (
                    r["game_date"],
                    game_id,
                    r["provider"],
                    r["sportsbook"],
                    r["market"],
                    r["home_team"],
                    r["away_team"],
                    r["home_price"],
                    r["away_price"],
                    r["commence_time"],
                    json.dumps(r["raw"]),
                ),
            )
            inserted += 1

    conn.commit()
    print(f"Inserted {inserted} odds rows for {game_date}")
    if skipped_unmapped:
        print(f"Skipped {skipped_unmapped} unmapped rows for {game_date}")
    return inserted


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--pg_dsn", default=os.environ.get("PG_DSN"))
    ap.add_argument("--provider", default="the-odds-api")
    ap.add_argument("--sportbook_filter", default="", help="comma-separated, optional (deprecated, use --bookmakers)")
    ap.add_argument("--bookmakers", default="", help="comma-separated list of bookmakers to filter")
    ap.add_argument("--game_date", help="YYYY-MM-DD (single date, ignored if --start/--end provided)")
    ap.add_argument("--start", help="YYYY-MM-DD (start of date range)")
    ap.add_argument("--end", help="YYYY-MM-DD (end of date range)")
    ap.add_argument("--sleep", type=float, default=0.0, help="Seconds to sleep between API calls")
    args = ap.parse_args()

    if not args.pg_dsn:
        raise RuntimeError("Missing PG_DSN")

    if not args.bookmakers and args.sportbook_filter:
        args.bookmakers = args.sportbook_filter

    if args.start and args.end:
        start_date = date.fromisoformat(args.start)
        end_date = date.fromisoformat(args.end)
        dates = []
        current = start_date
        while current <= end_date:
            dates.append(current)
            current += timedelta(days=1)
    elif args.game_date:
        dates = [date.fromisoformat(args.game_date)]
    else:
        dates = [date.today()]

    print(f"Processing {len(dates)} date(s) from {dates[0]} to {dates[-1]}")

    client = TheOddsAPI()
    conn = psycopg2.connect(args.pg_dsn)

    total_rows = 0
    processed_dates = 0

    for i, game_date in enumerate(dates):
        rows = process_date(client, game_date, args, conn)
        processed_dates += 1
        total_rows += rows

        if args.sleep > 0 and i < len(dates) - 1:
            print(f"Sleeping {args.sleep} seconds...")
            time.sleep(args.sleep)

    conn.close()
    print(f"\n{'='*60}")
    print("Summary:")
    print(f"  Total dates processed: {processed_dates}")
    print(f"  Total odds rows inserted: {total_rows}")
    print(f"{'='*60}")


if __name__ == "__main__":
    main()